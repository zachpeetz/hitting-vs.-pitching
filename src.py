...
# load datasets
hitting_df = pd.read_csv("\Users\zspee\OneDrive\Desktop\PersonalProjects\hitting-vs.-pitching\hitting.csv")
pitching_df = pd.read_csv("\Users\zspee\OneDrive\Desktop\PersonalProjects\hitting-vs.-pitching\pitching.csv")
...
hitting_df.rename(columns={'R': 'RS'}, inplace=True)
hitting_df.rename(columns={'WAR': 'bWAR'}, inplace=True)
...
# merge datasets
baseball_df = pd.merge(hitting_df, pitching_df, on=['Season', 'Team'], how='outer')

# create new columns
baseball_df['UER'] = baseball_df['RA'] - baseball_df['ER']
baseball_df['G'] = baseball_df['W'] + baseball_df['L']
baseball_df['wPCT'] = round((baseball_df['W']/baseball_df['G']), 3)
...
print('------- Missing Data -------')
print(baseball_df.isnull().sum())

# missing data visualization
msno.matrix(baseball_df)

missing_df = pd.DataFrame(index=list(baseball_df.columns))
missing_df['% Missing Data'] = (baseball_df.isnull().sum() / len(baseball_df))
...
### 2. EDA ###
# data structure
print('Data Structure: {}'.format(baseball_df.shape))
...
for col, ax in zip(ind_vars, axes.flatten()[:7]):
    sns.regplot(col, 'wPCT', data=baseball_df, scatter_kws={'color':'black'},
...
x = baseball_df.drop(['Season', 'Team', 'wPCT', 'Era'], axis=1)
y = baseball_df['wPCT']
